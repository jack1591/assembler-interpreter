...
inline string read_input(string path_to_log, string path_to_bin,string path_to_input,string name_of_file){
    ...
    ofstream binFile(path_to_bin+"output.bin",ios::binary | ios::binary);
    log << "<log>\n";
    ...
    while (getline(fin,s)){
        ...
        out+="\n";
        for (auto letter:out){
            binFile.write((char*)(&letter), sizeof(letter));
        }
        //binFile<<out;
        out = log_in(arguments,out.substr(0,out.size()-2),command);
        log<<out;
        length.clear();
    }
